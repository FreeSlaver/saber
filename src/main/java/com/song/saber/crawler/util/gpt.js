(function () {
  ...
  var r = q.getInstance().a, t = g(), u = t._vars_, m;
  for (m in u)r[m] = u[m];
  t._vars_ = r;
  var e = function () {
    return "122"
  ...
  var w = "", x = "", k = q.getInstance().get(46) && !q.getInstance().get(6),
      w = k ? "http:" : "https:", x = q.getInstance().get(k ? 2 : 3);
  var l = g(), n = l.fifWin || window, p = n.document, v = [], y = g();
  y.hasOwnProperty("cmd") || (y.cmd = v);
  if (l.evalScripts) {
    l.evalScripts();
  } else {
    var z = p.currentScript, A;
    var B = q.getInstance().get(76);
    if (B) {
      A = B;
    } else {
      var C = !!q.getInstance().get(131), D = "", E = x, F = w;
      C && (D = "?sf=1");
    ...
    -1
    != (window.navigator && window.navigator.userAgent || "").indexOf("iPhone")
    && q.getInstance().set(79, 0);
    var G = n.performance;
    if (G && G.now) {
      var H = G.now();
    ...
    if (!("complete" == p.readyState || "loaded" == p.readyState || z && z.async)) {
